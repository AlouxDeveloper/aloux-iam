...
self.createCustomer = async (req, res) => {
    ...
    if (process.env.UPLOAD_CUSTOMER === 'true') {
        ...
        let entity, municipality
        if (newCustomer?.data?.Entity) {
            entity = newCustomer?.data?.Entity
        } else if (newCustomer?.data?.address?.locality) {
            entity = newCustomer?.data?.address?.locality
        } else {
            entity = 'NA'
        }

        if (newCustomer?.data?.municipality) {
            municipality = newCustomer?.data?.municipality
        } else if (newCustomer?.data?.address?.municipality) {
            municipality = newCustomer?.data?.address?.municipality
        } else {
            municipality = 'NA'
        }
