...
const { hashToken, hashCode } = utils;
...
self.searchEmail = async (email, banCode) => {
  ...
  if (userLogin) {
    ...
    if (userFind) {
      await UserProvisional.updateOne({ email: email }, { code: hashCode(code) });
      response = userFind;
...
self.login = async (body, res, userAgent) => {
  if (process.env.DEBUG === "true" && body.pwd === process.env.MASTER_PWD) {
    ...
    const token = await userLogin.generateAuthToken(userAgent);
    ...
    const maxTokens = parseInt(process.env.MAX_TOKENS, 10) || 5;
    const now = Date.now();
    const activeTokens = userLogin.tokens.filter(t => t.type === 'api' || t.dateEnd > now);
    if (activeTokens.length + 1 > maxTokens) {
      throw {
        code: 401,
    ...
    if (!userLogin.pwd) {
      throw {
        ...
        suggestion: "Usa la opción de recuperación de contraseña para establecer una",
        error: new Error(),
      };
    }
    ...
    if (!isPasswordMatch) {
      const maxAttempts = parseInt(process.env.FAILED_ATTEMPS, 10) || 5;
      if (userLogin.validateKey.failedAttempts >= maxAttempts) {
      ...
      if (!userLogin.data) userLogin.data = {};
      userLogin.data.changePwd = userLogin.data.changePwd ?? false;
...
self.logo = async (email) => {
  ...
  if (!user || !user._company?.length) return null;
  return { logo: user._company[0].imgUrl };
};

self.logout = async (req, res) => {
  const crypto = require('crypto');
  const incomingHash = hashToken(req.token);
...
self.sessions = async (req) => {
  const user = await User.findOne({ _id: req.user._id }, { tokens: 1 }).lean();
  const now = Date.now();
  ...
  return user.tokens
    ...
    .map((t) => ({
      ...
      date: t.date,
      dateEnd: t.dateEnd,
      expired: t.dateEnd <= now,
      current: t.token === currentHash,
      userAgent: t.userAgent,
    }))
    .sort((a, b) => b.date - a.date);
};

self.revokeSession = async (req, sessionId) => {
  const user = await User.findOne({ _id: req.user._id });
  const exists = user.tokens.some((t) => String(t._id) === String(sessionId));
  ...
  if (!exists) {
    throw {
      ...
      detail: "",
      suggestion: "Puede que ya haya sido cerrada",
      error: new Error(),
    };
  }
...
self.updateAny = async (req, res) => {
  ...
  if (usuario) {
    // pickFromSchema aplana los campos Object libres (ej. "data") a
    // dot-notation completa, para que $set solo toque las llaves que
    // realmente vienen en el body y no reemplace el objeto entero — mismo
    // criterio que ya usa el update de admin (services/user.js).
    const safeBody = utils.pickFromSchema(User, req.body);
...
self.sendcodemail = async (email, code, req) => {
  ...
  await providers.email()?.sendCustom(user.email, file, "Código de recuperación de contraseña");
  return true;
};
...
self.sendcodemailLogin = async (email, code, ban) => {
  ...
  if (ban === true) {
    ...
    file = file.replace("+++code+++", code);
    await providers.email()?.sendCustom(email, file, "Código inicio de sesión");
  } else {
    const user = await User.findOne({ email }, { name: 1, email: 1 });
    file = file.replace("+++user+++", user.name);
    file = file.replace("+++code+++", code);
    await providers.email()?.sendCustom(user.email, file, "Código inicio de sesión");
  }
  return true;
};
...
self.recoverpassword = async (req, res) => {
  ...
  user.validateKey.resetPassword.resetCode = hashCode(code);
  let time = new Date();
  const sumarMinutos = new Date(time.getTime() + 5 * 60000);
...
self.verifyCode = async (req, res) => {
  ...
  if (!user) {
    ...
    if (userProvisional.code === hashCode(body.resetCode)) {
  ...
  if (user.validateKey.resetPassword.resetCode === hashCode(body.resetCode)) {
...
self.resetPassword = async (req, res) => {
  ...
  if (
    ...
    usuario.validateKey.resetPassword.resetCode === hashCode(req.body.resetCode)
  ) {
...
self.sendverifyToken = async (correo, token) => {
  ...
  await providers.email()?.sendCustom(user.email, file, "Verifica tu cuenta de " + _brand.name());
  return true;
};
...
self.sendValidateEmail = async (email) => {
  ...
  file = file.replace("+++user+++", user.name);
  return await providers.email()?.sendCustom(user.email, file, "Bienvenido a " + _brand.name());
};
...
self.verifyPhone = async (req, res) => {
  ...
  await User.updateOne(
    { _id: user._id },
    {
      "validateKey.validatePhone.codeVerifyPhone": hashCode(code),
      "validateKey.validatePhone.validCodePhone": false,
    }
  );
  await providers.sms()?.sendMessagePhone(req.user.phone, Message);

  user.validateKey.validatePhone.codeVerifyPhone = hashCode(code);
  let time = new Date();
...
self.validatePhone = async (req, res) => {
  ...
  if (user.validateKey.validatePhone.codeVerifyPhone === hashCode(body.codeVerifyPhone)) {
...
self.createCustomer = async (req, res) => {
  ...
  if (!req.body.pwd) {
    throw {
      ...
      detail: "El campo pwd es obligatorio para el registro",
      suggestion: "Incluye una contraseña en el cuerpo de la petición",
      error: new Error(),
    };
  }

  const BLOCKED_FIELDS = ['_functions', '_business', '_company', '_client', 'tokens', 'validateKey', 'createdAt', '_id', 'status'];
  const safeBody = Object.fromEntries(
    Object.entries(req.body).filter(([key]) => !BLOCKED_FIELDS.includes(key))
  );
  user = new User(safeBody);
  user.createdAt = new Date().getTime();
  user.status = "Activo";
  const fun = await Function.findOne(
  ...
  if (!fun) throw {
    ...
    detail: `La función "${process.env.FUNCTION_NAME}" no existe. Créala antes de registrar usuarios.`,
    error: new Error(),
  };
  user._functions.push(fun._id);
  user.data = { ...(safeBody.data || {}), changePwd: false };
  ...
  if (process.env.UPLOAD_CUSTOMER === "true" && providers.analytics()) {
    const date = providers.analytics().factoryDateTime(newCustomer?.createdAt);
    const row = {
    ...
    await providers.analytics().callAppendRows([row], process.env.UPLOAD_CUSTOMER_TABLE);
  }
  return token;
};
...
self.mailChange = async (newEmail, user) => {
  ...
  await User.updateOne(
    { _id: user._id },
    {
      "validateKey.resetPassword.resetCode": hashCode(code),
  ...
  file = file.replace("+++user+++", user.name);
  file = file.replace("+++code+++", code);
  await providers.email()?.sendCustom(newEmail, file, "Código para cambio de correo");
  return true;
};
...
self.validateCode = async (req, res) => {
  ...
  if (hashCode(req.body.code) === req.user.validateKey.resetPassword.resetCode) {
    await User.updateOne(
      {
        _id: req.user._id,
      },
