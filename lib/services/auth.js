...
self.verifyCode = async (req, res) => {
    ...
    if (!user) {
        const userProvisional = await UserProvisional.findOne({ email: correo },{code: 1})
        if(!userProvisional){
        ...
        if(userProvisional.code === body.resetCode){
            return {message: 'Usuario provisional'}
        }else{
...
self.createCustomer = async (req, res) => {
    ...
    user.status = 'Activo'
    let fun = await Function.findOne({ name: process.env.FUNCTION_NAME })
