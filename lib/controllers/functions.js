...
self.retrieve = async(req, res) => {    
    try {
        ...
        // ********* Mejora ********
        if((await user.getMe(req, res)).permission['iam.permission.propietario'])
            consulta = await Functions.find({isActive: true}).sort({createdAt:-1})
        else
            consulta = await Functions.find({'name': {$ne:'Propietario'}, isActive: true}).sort({createdAt:-1})
