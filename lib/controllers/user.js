...
const sesSDK = require("./operationsAWS") 
const fs = require('fs')
...
self.sendcodemail = async( email, code) =>{
    try {
        ...
        let file = fs.readFileSync(process.env.TemplateRecoverPassword, "utf8")
        file = file.replace('+++user+++', user.name)
        file = file.replace('+++code+++', code)
    
        return await sesSDK.sendCustom(user.email, file, "Código de recuperación de contraseña");
...
self.verifyCode = async (req, res) => {
    try {
        ...
        {
            user.validateKey.resetPassword.resetCode = null;
            user.validateKey.limitCodeTime = null;
...
self.resetPassword  =  async(req, res) => {    
    try {
        ...
        if(usuario.validateKey.resetPassword.validCode)
        {
            usuario.pwd = body.pwd;
            usuario.resetPassword.resetPassword.validCode = false;
            usuario.lastUpdate = new Date().getTime();
            usuario.tokens = []
...
self.verifyPhoneNumber = async (req, res) => {
    ...
    try {
        ...
        await sesSDK.sendMessagePhone(phone, Message)
...
self.validatePhoneNumber = async (req, res) => {
    try {
        const phone = req.user.phone
        var body = JSON.parse(JSON.stringify(req.body));
...
self.sendverifyToken = async (correo, token) => {
    try {
        ...
        let file = fs.readFileSync(process.env.TemplateVerifyEmail, "utf8")
        file = file.replace('+++user+++', user.name)
        file = file.replace('+++token+++', token)


        return await sesSDK.sendCustom(user.email, file, "Verificar Cuenta");
    } catch (error) {
        throw new Error('Ocurrio un error al envìar el correo electronico');
    }
}
...
self.sendVerifyMailAccount = async (req, res) => {
    ...
    try {
        ...
        const sendMail = await self.sendverifyToken(user.email, token);


        res.status(200).send('Correo enviado exitosamente, revisa tu bandeja de entrada y sigue las instrucciones');
    } catch (error) {
        res.status(500).send({ error: error.message })
    }
}
...
self.verifyMailTokenAccount = async (req, res) => {
    try {
        ...
        const user = await User.findOne({ _id: data._id, 'validateKey.validateEmail.verifyMailToken': token})
