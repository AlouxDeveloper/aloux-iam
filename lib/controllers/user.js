...
self.create = async (req, res) => {
  try {
    ...
    if (req.body.sendEmail === true && process.env.SEND_EMAIL_USER === "true" && user.email) {
      try {
        const [businesses, functions] = await Promise.all([
          Business.find({ _id: { $in: req.body._business || [] } }, { name: 1 }).lean(),
          Functions.find({ _id: { $in: req.body._functions || [] } }, { name: 1 }).lean(),
        ])
        ...
        file = file.replace("{{business}}", businesses.map((b) => b.name).join(", ") || "—")
        file = file.replace("{{function}}", functions.map((f) => f.name).join(", ") || "—")
