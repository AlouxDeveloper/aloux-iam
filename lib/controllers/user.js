...
self.get  =  async(req, res) => {    
    try {
        const _id  = req.params.USER_ID
        const user = await User.findOne({_id}, {pwd:0}).populate([{ path: "_functions"}, { path: "_client"}]).select("-pwd -tokens").lean()
...
self.retrieve = async(req, res) => {    
    try {
        let result = await User.find({}).select("-pwd -tokens").populate([{ path: "_functions" },{ path: "_client"}]).sort({createdAt:-1}).lean()
